...
ASE Calculator wrapper for a MACE model ensemble.
- Loads all MACE models for a given iteration from runs/iterXXX/boot_XXX/checkpoints/
- Computes ensemble-averaged energy and forces
- Calculates per-atom energy uncertainty (sigma_E_pa) and mean force uncertainty (sigma_F_mean)
- Stores uncertainties in atoms.info["key_value_pairs"] for downstream GA or analysis scripts

Usage:
    from committee_calc import CommitteeCalculator
    calc = CommitteeCalculator(iteration=0)
...
class CommitteeCalculator(Calculator):
    ...
    def __init__(self, iteration: int, use_forces=True, **kwargs):
        ...
        pattern = str(Path("runs")/ f"iter{iteration:03d}" / "boot_*" / "checkpoints" / f"MACE_iter{iteration:03d}_boot*_run-123_stagetwo.model")
