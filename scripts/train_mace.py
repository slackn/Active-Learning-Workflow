#!/usr/bin/env python3
"""
train_mace_ensemble.py

Train an ensemble of MACE models for a given active learning iteration. 
- Reads project config from config.yaml (base MACE settings, test set, run dirs)
- Reads manifest.json for iteration number, bootstrapped train sets, and validation set
- Writes per-model config.yaml files into runs/iterXXX/boot_XXX/
- Runs mace_run_train sequentially for each model (can be parallelized via SLURM arrays)

Usage:
    python train_mace_ensemble.py --config config.yaml --manifest data/iter000/manifest.json
"""
