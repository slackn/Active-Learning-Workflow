...
Merge existing dataset with newly DFT-labeled structures for the next iteration.
- Reads previous iteration dataset and DFT-relaxed structures
- Optionally includes failed DFT relaxations
- Deduplicates structures based on a key (default 'confid')
- Writes merged dataset to data/iterXXX+1/dataset_iterXXX+1.xyz

Usage:
    python merge_datasets.py --iter 0 [--include_failed]
"""
...
def merge_datasets(iter_idx: int, include_failed: bool = False, dedup_key: str = "confid"):
    ...
    key = dedup_key
    seen, merged = set(), []
    # The order matters because it decides which to keep
    for a in (relaxed_frames + prev_frames  + failed_frames):
        val = a.info.get(key)
        # if confid is missing or "N/A", just treat it as None
