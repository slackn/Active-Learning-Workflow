...
Generate the initial random population for a GA iteration.
- Reads project configuration from config.yaml
- Creates an empty simulation cell (slab) and places atoms randomly
- Ensures minimum interatomic distances
- Writes unrelaxed candidates to a new ASE database for the iteration

Usage:
    python create_initial_db.py --config config.yaml --iter 0
"""
