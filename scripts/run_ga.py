...
Run a genetic algorithm (GA) to generate candidate structures for DFT labeling using ASE.
- Reads project configuration from config.yaml
- Initializes a population of random structures and relaxes them with a MACE committee calculator
- Evolves the population through pairing and mutations
- Relaxes offspring and updates the database
- Selects top structures based on energy uncertainty and writes them to data/iterXXX/selected_for_dft.extxyz

Usage:
    python run_ga.py --config config.yaml --iter 0
"""
